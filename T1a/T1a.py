...
FONT_WEIGHT = 'normal'
FONT_STYLE = 'normal'
plt.rcParams.update({'font.family': FONT_FAMILY, 'font.weight': FONT_WEIGHT, 'font.style': FONT_STYLE, 'mathtext.fontset': 'dejavusans', 'axes.titlesize': FONT_TITLE, 'axes.labelsize': FONT_LABEL, 'axes.titleweight': FONT_WEIGHT, 'axes.labelweight': FONT_WEIGHT, 'xtick.labelsize': FONT_TICK_AXIS, 'ytick.labelsize': FONT_TICK_AXIS, 'legend.fontsize': FONT_TICK_AXIS, 'axes.unicode_minus': False})
VIEW_ELEV = 28
VIEW_AZIM = -120
THREE_D_CMAP = 'jet'
SINGULAR_EPS = 1e-10
...
OUTDIR = 'phase_shg_outputs_mpi_v6_full_T1a_tickstyle_matched'
PHASE_TOL = 1e-08
SAVE_NPY = True
...
def _set_text_font_normal(text_obj, fontsize: int) -> None:
    text_obj.set_fontsize(fontsize)
    text_obj.set_fontfamily(FONT_FAMILY)
    text_obj.set_fontweight(FONT_WEIGHT)
    text_obj.set_fontstyle(FONT_STYLE)

def apply_unified_axis_font(ax, include_z: bool=False) -> None:
    ...
    for tick_label in ax.get_xticklabels(which='both') + ax.get_yticklabels(which='both'):
        _set_text_font_normal(tick_label, FONT_TICK_AXIS)
    if include_z and hasattr(ax, 'zaxis'):
        ax.zaxis.label.set_size(FONT_LABEL)
        ax.zaxis.label.set_fontweight(FONT_WEIGHT)
        ax.tick_params(axis='z', labelsize=FONT_TICK_AXIS)
        for tick_label in ax.get_zticklabels():
            _set_text_font_normal(tick_label, FONT_TICK_AXIS)

def apply_unified_cbar_font(cbar) -> None:
    cbar.ax.tick_params(labelsize=FONT_TICK_CBAR)
    for tick_label in cbar.ax.get_yticklabels(which='both') + cbar.ax.get_xticklabels(which='both'):
        _set_text_font_normal(tick_label, FONT_TICK_CBAR)
...
def save_heatmap_shg(x_vec: np.ndarray, y_vec: np.ndarray, data_beta_alpha: np.ndarray, title: str, fpath: Path) -> None:
    ...
    ax.tick_params(axis='both', labelsize=FONT_TICK_AXIS)
    apply_unified_axis_font(ax)
    cbar = fig.colorbar(im, ax=ax)
    apply_unified_cbar_font(cbar)
    fig.tight_layout()
...
def save_3d_surface_shg(x_vec: np.ndarray, y_vec: np.ndarray, data_beta_alpha: np.ndarray, title: str, fpath: Path, elev: float=VIEW_ELEV, azim: float=VIEW_AZIM) -> None:
    ...
    apply_unified_axis_font(ax, include_z=True)
...
def write_phase_summary_workbook(summary_rows: List[Dict[str, float]], filepath: Path) -> None:
    ...
    headers = ['phase', 'point_count', 'T1a_sum', 'T1a_mean']
    ws.append(headers)
...
def summarize_by_phase(phase_masks: Dict[str, np.ndarray], T1a_abs_real: np.ndarray) -> List[Dict[str, float]]:
    rows: List[Dict[str, float]] = []
    for phase_label, mask in phase_masks.items():
        count = int(np.count_nonzero(mask))
        if count == 0:
            rows.append({'phase': phase_label, 'point_count': 0, 'T1a_sum': np.nan, 'T1a_mean': np.nan})
            continue
        t1a_sum = float(np.nansum(T1a_abs_real[mask]))
        rows.append({'phase': phase_label, 'point_count': count, 'T1a_sum': t1a_sum, 'T1a_mean': t1a_sum / count})
    return rows
...
def _normalize_density_on_grid(density: np.ndarray, xs: np.ndarray) -> np.ndarray:
    ...
    if xs.size < 2:
        return density
    area = float(np.trapz(density, xs))
...
def build_phase_point_rows(phase_masks: Dict[str, np.ndarray], alpha_vec: np.ndarray, beta_vec: np.ndarray, average_shift_grid_alpha_beta: np.ndarray, T1a_abs_real_beta_alpha: np.ndarray) -> List[Dict[str, float]]:
    rows: List[Dict[str, float]] = []
    for phase_label, mask in phase_masks.items():
        ib_ia_pairs = np.argwhere(mask)
        for ib, ia in ib_ia_pairs:
            rows.append({'phase': phase_label, 'ia': int(ia), 'ib': int(ib), 'alpha': float(alpha_vec[ia]), 'beta': float(beta_vec[ib]), 'average_shift': float(average_shift_grid_alpha_beta[ia, ib]), 'T1a_abs_real': float(T1a_abs_real_beta_alpha[ib, ia])})
    return rows
...
def save_outputs(cfg: Config, topo_calc: TopoPhaseCalculator, topo_results: Dict[str, np.ndarray], shg_maps: Dict[str, np.ndarray], summary_rows: List[Dict[str, float]], distribution_rows: List[Dict[str, float]], phase_masks: Dict[str, np.ndarray]) -> None:
    ...
    T1a_abs_real = np.abs(np.real(shg_maps['T1a']))
    save_phase_hist_kde(phase_masks, T1a_abs_real, '$|\\mathrm{Re}[T1a]|$', analysis_dir / 'hist_kde_T1a.png')
    save_phase_hist_kde_two_rows_single_observable(phase_masks, T1a_abs_real, '$|\\mathrm{Re}[T1a]|$', analysis_dir / 'hist_kde_T1a_by_phase_rows.png')
    save_phase_kde_only(phase_masks, T1a_abs_real, '$|\\mathrm{Re}[T1a]|$', analysis_dir / 'kde_T1a.png')
    save_phase_mean_barplot(summary_rows, 'T1a_mean', 'Mean $|\\mathrm{Re}[T1a]|$', analysis_dir / 'mean_T1a_by_phase.png', 'Mean of $|\\mathrm{Re}[T1a]|$ by phase')
    for phase_label, phase_mask in phase_masks.items():
        ...
        save_phase_masked_heatmap(alpha_vec, beta_vec, T1a_abs_real, phase_mask, f'$|\\mathrm{{Re}}[T1a]|$ in {phase_label}', analysis_dir / f'T1a_masked_{phase_tag}.png')
    ...
    phase_point_rows = build_phase_point_rows(phase_masks, alpha_vec, beta_vec, average_shift_grid, T1a_abs_real)
    write_records_csv(phase_point_rows, phase_points_csv)
    write_phase_point_workbook(phase_point_rows, phase_points_xlsx)
    t1a_table = grid_to_table(alpha_vec, beta_vec, np.abs(np.real(shg_maps['T1a'])).T)
    ...
    write_grid_csv(t1a_table, tables_dir / 'T1a_intensity_grid.csv')
...
def main() -> None:
    ...
    for ia, ib, alpha, beta in my_tasks:
    ...
    summary_rows = summarize_by_phase(phase_masks, T1a_abs_real)
    distribution_rows: List[Dict[str, float]] = []
    distribution_rows.extend(distribution_stats_by_phase(phase_masks, T1a_abs_real, 'T1a_abs_real'))
    save_outputs(cfg, topo_calc , topo_results, shg_maps, summary_rows, distribution_rows, phase_masks)
